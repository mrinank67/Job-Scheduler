...
export function ChronoPrintApp() {
  ...
  const calculateNextRun = useCallback((schedule: Omit<Schedule, 'nextRun' | 'id' | 'createdAt'> & { nextRun?: Schedule['nextRun'] }, fromTime: Date = new Date()): Date | undefined => {
    if (!schedule.isEnabled) return undefined;

    let baseTime = new Date(fromTime); // Use a mutable copy

    const [hour, minute] = schedule.startTime.split(':').map(Number);
    let nextRunCand = setMilliseconds(setSeconds(setMinutes(setHours(new Date(baseTime), hour), minute), 0), 0);
    ...
    if (schedule.type !== 'Hourly' && isAfter(baseTime, nextRunCand)) {
        ...
        nextRunCand = setMilliseconds(setSeconds(setMinutes(setHours(nextRunCand, hour), minute),0),0);
    }
    ...
    switch (schedule.type) {
      case 'Hourly':
        ...
        let candidate = setMilliseconds(setSeconds(setMinutes(setHours(new Date(baseTime), hour), minute),0),0);
        ...
        if (isAfter(baseTime, candidate)) {
             ...
             if (candidate.getMinutes() % schedule.interval !== minute % schedule.interval) {
                 candidate = setMinutes(candidate, (Math.floor(candidate.getMinutes()/schedule.interval) * schedule.interval) + (minute % schedule.interval) );
                 if (candidate.getMinutes() >= 60) {
                     candidate = addMinutes(candidate, schedule.interval - (candidate.getMinutes() - (minute % schedule.interval)))
            ...
            candidate = setMilliseconds(setSeconds(setMinutes(setHours(new Date(baseTime), hour), minute),0),0);
        }

        while (!isAfter(candidate, baseTime) || candidate.getMinutes() % schedule.interval !== minute % schedule.interval) {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      setSchedules(fetchedSchedules.map(s => ({...s, nextRun: s.nextRun ? (s.nextRun as Timestamp).toDate() : undefined })));
      setExecutionLogs(fetchedLogs.map(l => ({...l, executionTime: (l.executionTime as Timestamp).toDate()})));
      setIsLoading(false);
    };
    loadData();
  }, []);
  ...
  const handleAddSchedule = useCallback(async (newScheduleData: Omit<Schedule, 'id' | 'nextRun' | 'isEnabled' | 'createdAt'> & { interval?: number }) => {
    const scheduleBase = {
      ...newScheduleData,
      isEnabled: true,
    };
    ...
    const scheduleWithNextRun: Omit<Schedule, 'id' | 'createdAt'> & { nextRun?: Date } = {
        ...scheduleBase,
        nextRun: nextRunTime,
    };

    const addedSchedule = await addScheduleToFirestore(scheduleWithNextRun as any); // Cast as any because firestore service expects specific shape
    if (addedSchedule) {
  ...
  const handleToggleSchedule = useCallback(async (scheduleId: string, isEnabled: boolean) => {
    const scheduleToUpdate = schedules.find(s => s.id === scheduleId);
    if (!scheduleToUpdate) return;

    const nextRunTime = isEnabled ? calculateNextRun({ ...scheduleToUpdate, isEnabled }, new Date()) : undefined;
    const success = await updateScheduleInFirestore(scheduleId, { isEnabled, nextRun: nextRunTime });
    ...
    if (success) {
      ...
      toast({
        ...
        description: `${scheduleToUpdate.jobName} is now ${isEnabled ? 'active' : 'paused'}.`,
      });
    } else {
      toast({
        title: "Error",
  ...
  const handleDeleteSchedule = useCallback(async (scheduleId: string) => {
    ...
    if (success) {
      ...
      setExecutionLogs(prev => prev.filter(log => log.jobId !== scheduleId)); // Also remove logs from UI
      toast({
  ...
  const addLogEntry = useCallback(async (log: Omit<LogEntry, 'id'>) => {
    const executionTime = log.executionTime instanceof Date ? log.executionTime : new Date();
    const newLogEntryData = { ...log, executionTime };

    const addedLog = await addLogToFirestore(newLogEntryData);
    if (addedLog) {
      setExecutionLogs(prevLogs => [...prevLogs, addedLog].sort((a,b) => (b.executionTime as Date).getTime() - (a.executionTime as Date).getTime()));
    }
  ...
  useEffect(() => {
    const intervalId = setInterval(async () => {
      ...
      for (const schedule of schedules) {
        if (schedule.isEnabled && schedule.nextRun && isAfter(now, schedule.nextRun as Date)) {
          ...
          const nextRunTimeForThisSchedule = calculateNextRun(schedule, schedule.nextRun as Date);
          
          const success = await updateScheduleInFirestore(schedule.id, { nextRun: nextRunTimeForThisSchedule });
          if (success) {
            ...
            console.error(`Failed to update nextRun for schedule ${schedule.jobName}`);
          }
        }
      }
